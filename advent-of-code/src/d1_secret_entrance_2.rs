...
pub fn secret_entrance_2() {
    ...
    for line in reader.lines() {
        ...
        let value_i16 = value.parse::<i16>().unwrap();

        let chunks = value_i16 / MAX_SAFE_VALUE;
        let remainder = value_i16 - MAX_SAFE_VALUE * chunks;

        match direction {
            "L" => {
                safe_pointer -= remainder;
                if safe_pointer < 0 && !last_is_zero {
                    password += 1;
                }
            }
            "R" => {
                ...
                if safe_pointer > MAX_SAFE_VALUE && !last_is_zero {
                    password += 1;
                }
            }
        ...
        if safe_pointer == 0 && remainder != 0 {
            password += 1;
        }

        last_is_zero = safe_pointer == 0;
    }

    println!("d1.2 The password is {}", password);
}
